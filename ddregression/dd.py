...
def dd(c_pass, c_fail, test):
    ...
    while 1:
        print("long loop")
        assert isinstance(test(c_pass), Passed)
        assert isinstance(test(c_fail), Failed)
        ...
        while j < n:
            ...
            if isinstance(test(next_c_fail), Failed) and n == 2:  # (1)
                print("1")
                c_fail = next_c_fail
                n = 2
                offset = 0
                break
            elif isinstance(test(next_c_fail), Passed):  # (2)
                print("2")
                c_pass = next_c_fail
                n = 2
                offset = 0
                break
            elif isinstance(test(next_c_pass), Failed):  # (3)
                print("3")
                c_fail = next_c_pass
                n = 2
                offset = 0
                break
            elif isinstance(test(next_c_fail), Failed):  # (4)
                print("4")
                c_fail = next_c_fail
                n = max(n - - 1, 2)
                offset = i
                break
            elif isinstance(test(next_c_pass), Passed):  # (5)
                print("5")
                c_pass = next_c_pass
                n = max(n - - 1, 2)
                offset = i
